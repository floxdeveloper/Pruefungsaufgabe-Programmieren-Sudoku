public class sudoku {
 ...
 public static void sudokuBT() {
  ...
  if(xkoord == -1) // nichts mehr auszufüllen -> fertig
   ausgabe();  
  else{ //backtracking
 ...
 public static boolean isSafe(int hor, int ver, int zahl) {    // Prüft ob eine Zahl an der Stelle hor/ver eingesetzt werden darf
 ...
 public static int[] naechsteKoordinaten() {
   for (int j = 0; j < sudoku.length; j++) {
    for (int i = 0; i < sudoku.length; i++) {
    if (sudoku[i][j] == 0)
     return new int[] { i, j }; // nächste koordinate, die probiert werden muss
   }
  }
